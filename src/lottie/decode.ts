...
export function loadLottie(src: string): Promise<LottieData> {
  let pending = cache.get(src);
  if (!pending) {
    pending = fetchAndDecode(src).catch((err: unknown) => {
...
async function fetchAndDecode(src: string): Promise<LottieData> {
 ...
 */
export function decodeLottieBytes(
  bytes: Uint8Array,
  label = "lottie",
): LottieData {
  ...
  const isZip =
    ...
    bytes[2] === 0x03 &&
    bytes[3] === 0x04;
  if (isZip) return decodeDotLottie(bytes, label);
  return JSON.parse(decode(bytes)) as LottieData;
}

function decodeDotLottie(bytes: Uint8Array, label: string): LottieData {
  const files = unzipSync(bytes);
  const animPath = findAnimationPath(files, label);
  const lottie = JSON.parse(decode(files[animPath])) as LottieData;
  inlineDotLottieImages(lottie, files);
  return lottie;
}

function findAnimationPath(files: Unzipped, label: string): string {
  const manifestRaw = files["manifest.json"];
  let manifest:
    | { animations?: Array<{ id?: string }>; entry?: string }
    | undefined;
  if (manifestRaw) {
    try {
  ...
  if (manifest && typeof manifest.entry === "string" && files[manifest.entry]) {
    throw new Error(
      `[remotion-animation] ${label}: this is a Remotion animation bundle ` +
  ...
  const found = Object.keys(files).find((k) =>
    /^(animations|a)\/[^/]+\.json$/i.test(k),
  );
  if (!found) {
    throw new Error(
      `[remotion-animation] ${label}: no Lottie animation found inside the archive`,
    );
  }
  return found;
}
 ...
 * left untouched.
 */
function inlineDotLottieImages(lottie: LottieData, files: Unzipped): void {
  const assets = (lottie as { assets?: Array<Record<string, unknown>> }).assets;
  if (!Array.isArray(assets)) return;
  for (const asset of assets) {
    const p = asset.p;
    if (typeof p !== "string" || p.startsWith("data:")) continue; // already embedded

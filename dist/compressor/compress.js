...
import { ASSET_MODULE, BUNDLE_FORMAT, RUNTIME_EXTERNALS, } from "../core/manifest.js";
 ...
 */
export async function compressAnimation(animDir, options = {}) {
    ...
    if (!existsSync(entry))
        throw new Error(`${name}: missing composition.tsx`);
 ...
 */
export async function buildAnimations(options) {
    const { srcDir, outDir, onBuilt, ...compressOptions } = options;
    if (!existsSync(srcDir)) {
        throw new Error(`animations source directory not found: ${srcDir}`);
    }
    const names = readdirSync(srcDir).filter((n) => statSync(path.join(srcDir, n)).isDirectory());
    if (!existsSync(outDir))
        mkdirSync(outDir, { recursive: true });
    const results = [];
    for (const name of names) {
        const result = await compressAnimation(path.join(srcDir, name), compressOptions);
 ...
 */
function assetPlugin(animDir, collected) {
    return {
        ...
        setup(b) {
            ...
            b.onLoad({ filter: /.*/, namespace: "lf-asset" }, (args) => ({
                contents: `import { asset } from ${JSON.stringify(ASSET_MODULE)};\n` +
...
function smokeTest(code, id) {
    const requireStub = (s) => s === ASSET_MODULE ? { asset: () => "blob:stub" } : {};
    const mod = { exports: {} };
...
function resolveRemotionVersion() {
    try {

...
async function main() {
    ...
    const outDir = path.resolve(process.cwd(), outArg ?? path.join("public", "animations-zips"));
    const kb = (n) => `${(n / 1024).toFixed(1)}KB`;
    console.log(`Building animations\n  from ${srcDir}\n  to   ${outDir}\n`);
    const results = await buildAnimations({
        srcDir,
        outDir,
        onBuilt: (r) => console.log(`  ✓ ${r.id.padEnd(24)} js ${kb(r.jsBytes).padStart(8)}   ` +
            `zip ${kb(r.zipBytes).padStart(8)}   assets ${r.assetCount}`),
    });
    if (results.length === 0)
        console.log("  (no animation folders found)");
